...
x_train, x_test, y1_train, y1_test, y2_train, y2_test, pred_semantic_l1_train, pred_semantic_l1_test,\
        ...
        train_test_split(cont_pad, l1_1hot, l2_1hot, pred_semantic_l1_pad, pred_l1, test_size=0.2, random_state=42)
...
#######################################################################################################################
print('Train...')
fileweights = r"../log/wos/wos_weight_l2_without_l1.hdf5"
...
#           shuffle= True,
#           verbose= 2)
loss,accuracy = model.evaluate([x_test], y2_test, verbose=0)
print('\ntest loss',loss, 'accuracy',accuracy)

pred_l2_test =model.predict([x_test], batch_size=batch_size)#此处传入的X_test和Input层的内容一致，要是多个Input()，就传个列表，和model.fit传入的参数一致(不传y_test)
